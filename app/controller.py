...
def generate_initial_plan():
    ...
    try:
        ...
        if not plan_json:
            app.logger.error("Plan generation failed: Claude returned no content. Check /api/claude_ping and logs.")
            # ---- SAFE FALLBACK so you can keep testing the app ----
            app.logger.warning("Using fallback plan generator (Claude unavailable).")
            fallback = _fallback_plan_from_major(mandatory_units, optional_units, additional_units)

            # Save the plan (same as below)
            study_plan = StudyPlan(
                session_id=current_session_id,
                major_id=selected_major_id,
                plan_data=json.dumps(fallback),
                is_valid=True
            )
            db.session.add(study_plan)
            db.session.commit()

            # Enrich and collect electives using the helper
            enriched_plan, unused_major_electives, unique_general_electives = _enrich_and_collect_electives(
                fallback, major_unit_relationships, selected_major
            )
            ...
            return jsonify({
                ...
                'general_electives': unique_general_electives[:50],
                'major': {
                    'code': selected_major.code,
                    'name': selected_major.name,
                    'degree': selected_major.degree
                },
                'note': 'Generated by fallback planner (Claude unavailable)'
            })
...
def _fallback_plan_from_major(mandatory_units, optional_units, additional_units):
    """
    Simple deterministic planner:
    1) Place all mandatory cores by ascending level (L1→L2→L3)
    2) Then fill with optional units (level order)
    3) Then fill with additional units (level order)
    Ensures 4 units per semester × 6 semesters. No AI, no availability logic.
    """
    semesters = [
    ...
    def flatten_by_level(buckets):
        return buckets.get('level_1', []) + buckets.get('level_2', []) + buckets.get('level_3', [])

    pool = []
    pool += flatten_by_level(mandatory_units)
    pool += flatten_by_level(optional_units)
    pool += additional_units.get('level_1', [])[:30]
    pool += additional_units.get('level_2', [])[:30]
    pool += additional_units.get('level_3', [])[:30]
    ...
    idx = 0
    for s in semesters:
        while len(plan[s]) < 4 and idx < len(ordered):
            plan[s].append(ordered[idx])
            idx += 1

    # If still not full, pad with generic placeholders so UI continues
    if any(len(v) < 4 for v in plan.values()):
        fillers = [f"GENE100{i}" for i in range(1, 40)]
        fill_idx = 0
        for s in semesters:
            while len(plan[s]) < 4 and fill_idx < len(fillers):
                plan[s].append(fillers[fill_idx])
                fill_idx += 1

    return plan


def _enrich_and_collect_electives(plan_data, major_unit_relationships, selected_major):
    """Reuses your existing enrichment/electives logic so fallback returns the same shape as AI path."""
    enriched_plan = {}
    units_in_plan = set()
    for semester, unit_codes in plan_data.items():
        enriched_plan[semester] = []
    ...
    general_electives = []
    if 'economics' in degree_type:
        prefixes = ['ECON', 'FINA', 'ACCT', 'MGMT', 'EMPL', 'STAT', 'MATH', 'WILG']
        limit_each = 20
    else:
        prefixes = ['SCIE', 'BIOL', 'CHEM', 'PHYS', 'MATH', 'STAT', 'ENVS', 'GEOL', 'GEOG', 'AGRI', 'ANAT', 'PHYL', 'PCOL', 'GENET', 'MICR']
        limit_each = 15

    for p in prefixes:
        items = Unit.query.filter(
            Unit.code.like(f"{p}%"),
            Unit.is_bridging == False,
            ~Unit.code.in_(units_in_plan)
    ...
    for u in general_electives:
        if u['code'] not in seen_codes:
            ...
            unique_general.append(u)

    unique_general.sort(key=lambda x: (x['level'], x['code']))
    return enriched_plan, unused_major_electives, unique_general
